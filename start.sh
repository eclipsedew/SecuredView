#!/bin/bash
# Start SecuredView backend + Cloudflare quick tunnel.
# Never use bare `pkill -f "uvicorn main:app"` (that hits ollama too).
set -u
...
# Restart quick tunnel (URL changes each start)
...
for i in $(seq 1 30); do
  ...
  if [ -n "${TUN:-}" ]; then
    ...
    # Keep meridianglobal.site /api/* pointed at this tunnel
    if [ "${SKIP_VERCEL_SYNC:-0}" != "1" ]; then
